...
import os, io, json, base64, re, uuid, asyncio, traceback, datetime, time, logging
...
GOOGLE_REDIRECT_URI = os.getenv("GOOGLE_REDIRECT_URI", _default_redirect("/gmail/oauth2callback"))
GOOGLE_SCOPES = [
...
MS_REDIRECT_URI = os.getenv("MS_REDIRECT_URI", _default_redirect("/outlook/callback"))
...
if not OPENAI_API_KEY:
    raise RuntimeError("Set OPENAI_API_KEY in the environment before starting the server.")
AVAILABLE_SERVICES: Dict[str, bool] = {
    "google": all([GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_REDIRECT_URI]),
    "microsoft": all([MS_TENANT_ID, MS_CLIENT_ID, MS_CLIENT_SECRET, MS_REDIRECT_URI]),
}
if not any(AVAILABLE_SERVICES.values()):
    logging.warning(
        "No provider credentials configured; running without Gmail/Outlook connectivity. "
        "Set Google or Microsoft credentials to enable full functionality."
    )

SESSION_COOKIE_MAX_AGE = int(os.getenv("SESSION_COOKIE_MAX_AGE", str(60 * 60 * 24 * 365)))
SESSION_PERSISTENCE_ENABLED = _env_truthy(os.getenv("SESSION_PERSISTENCE_ENABLED"), default=True)
_SESSION_STORAGE_PATH: Optional[Path] = None
_PERSISTENT_STATE: Dict[str, Dict[str, Any]] = {}
if SESSION_PERSISTENCE_ENABLED:
    raw_session_path = os.getenv("SESSION_STORAGE_PATH", "session_store.json")
    if raw_session_path:
        try:
            _SESSION_STORAGE_PATH = Path(raw_session_path).expanduser().resolve()
            if _SESSION_STORAGE_PATH.exists():
                _PERSISTENT_STATE = json.loads(_SESSION_STORAGE_PATH.read_text())
        except Exception:
            logging.exception("Session persistence disabled; could not initialize storage.")
            _SESSION_STORAGE_PATH = None
            SESSION_PERSISTENCE_ENABLED = False
    else:
        SESSION_PERSISTENCE_ENABLED = False
        logging.info("Session persistence disabled via empty SESSION_STORAGE_PATH.")
elif os.getenv("SESSION_STORAGE_PATH"):
    try:
        _SESSION_STORAGE_PATH = Path(os.getenv("SESSION_STORAGE_PATH", "")).expanduser().resolve()
    except Exception:
        _SESSION_STORAGE_PATH = None
...
SESSION_SECRET = (
    ...
    or os.getenv("FASTAPI_SESSION_SECRET")
    or os.getenv("SECRET_KEY")
)
if not SESSION_SECRET:
    SESSION_SECRET = "dev-session-secret-change-me"
    logging.warning("SESSION_SECRET not set; using insecure development default.")
app.add_middleware(
    SessionMiddleware,
...
_MAX_AUDIO_CACHE = int(os.getenv("MAX_AUDIO_CACHE_PER_SESSION", "10"))
...
def _ensure_session_id(store: Dict[str, Any]) -> str:
...
def _save_persistent_state() -> None:
    global SESSION_PERSISTENCE_ENABLED, _SESSION_STORAGE_PATH
    if not SESSION_PERSISTENCE_ENABLED or not _SESSION_STORAGE_PATH:
        return
    try:
        _SESSION_STORAGE_PATH.parent.mkdir(parents=True, exist_ok=True)
        _SESSION_STORAGE_PATH.write_text(json.dumps(_PERSISTENT_STATE, indent=2, sort_keys=True))
    except Exception:
        logging.exception("Failed to persist session storage to %s; disabling persistence.", _SESSION_STORAGE_PATH)
        SESSION_PERSISTENCE_ENABLED = False
        _SESSION_STORAGE_PATH = None
...
def _get_session_state(session_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    state.setdefault("audio_order", deque())
    persistent = _PERSISTENT_STATE.get(session_id) if SESSION_PERSISTENCE_ENABLED else None
    if persistent:
        if "google_creds" not in state and persistent.get("google_creds"):
            try:
                state["google_creds"] = Credentials.from_authorized_user_info(persistent["google_creds"])
            except Exception:
                logging.exception("Failed to restore Google credentials for session %s", session_id)
                persistent.pop("google_creds", None)
                _save_persistent_state()
        if "ms_token" not in state and persistent.get("ms_token"):
            state["ms_token"] = persistent["ms_token"]
    return state

def _set_google_creds(creds: Credentials) -> None:
    ...
    if SESSION_PERSISTENCE_ENABLED:
        try:
            ...
            logging.exception("Failed to serialize Google credentials for session %s", session_id)
            info = None
        if info:
            persistent = _PERSISTENT_STATE.setdefault(session_id, {})
            persistent["google_creds"] = info
            _save_persistent_state()

def _get_google_creds() -> Optional[Credentials]:
    return _get_session_state().get("google_creds")

def _clear_google_creds() -> None:
    session_id = _current_session_id()
    _get_session_state(session_id).pop("google_creds", None)
    if SESSION_PERSISTENCE_ENABLED:
        persistent = _PERSISTENT_STATE.get(session_id)
        if persistent and persistent.pop("google_creds", None) is not None:
            if not persistent:
                _PERSISTENT_STATE.pop(session_id, None)
            _save_persistent_state()

def _set_ms_token(token: Dict[str, Any]) -> None:
    session_id = _current_session_id()
    state = _get_session_state(session_id)
    state["ms_token"] = token
    if SESSION_PERSISTENCE_ENABLED:
        persistent = _PERSISTENT_STATE.setdefault(session_id, {})
        persistent["ms_token"] = token
        _save_persistent_state()

def _get_ms_token() -> Optional[Dict[str, Any]]:
    return _get_session_state().get("ms_token")

def _clear_ms_token() -> None:
    session_id = _current_session_id()
    _get_session_state(session_id).pop("ms_token", None)
    if SESSION_PERSISTENCE_ENABLED:
        persistent = _PERSISTENT_STATE.get(session_id)
        if persistent and persistent.pop("ms_token", None) is not None:
            if not persistent:
                _PERSISTENT_STATE.pop(session_id, None)
            _save_persistent_state()

def _store_audio_bytes(audio_bytes: bytes) -> str:
    state = _get_session_state()
    audio_cache: Dict[str, bytes] = state["audio_cache"]
    audio_order: deque = state["audio_order"]
    audio_id = str(uuid.uuid4())
    audio_cache[audio_id] = audio_bytes
    audio_order.append(audio_id)
    while len(audio_order) > _MAX_AUDIO_CACHE:
        expired = audio_order.popleft()
        audio_cache.pop(expired, None)
    return audio_id

def _get_audio_bytes(session_id: str, audio_id: str) -> Optional[bytes]:
    state = _SESSION_STATE.get(session_id)
    if not state:
        return None
    audio_cache: Dict[str, bytes] = state.get("audio_cache", {})
    return audio_cache.get(audio_id)
...
def _tool_status_message(name: str, args: Dict[str, Any], service: str) -> Optional[str]:
    ...
    if name in {"gmail_mark_all_read", "outlook_mark_all_read"}:
        return "Clearing every unread email in your inbox..."
    ...
    <div class="chat-container" id="chat-container">
      <div id="auth-view" class="auth-view" style="display:none;">
        ...
        <div class="auth-buttons" id="auth-buttons">
          <a id="connect-google" class="btn" href="/gmail/login">Connect Google</a>
          <a id="connect-outlook" class="btn" href="/outlook/login">Connect Outlook</a>
        </div>
      </div>
      <div id="chat-log"></div>
...
async function checkAuth(){
  ...
  try {
    ...
    if (authMsg) authMsg.textContent = 'Unable to reach the assistant. Refresh to try again.';
    if (authView) authView.style.display = 'block';
    if (controls) controls.style.display = 'none';
    updateStatus('Waiting for connection...');
    return;
  }
  const available = new Set(payload.available_services || []);
  const anyAvailable = available.size > 0;
  if (connectGoogle) connectGoogle.style.display = available.has('google') ? 'inline-flex' : 'none';
  if (connectOutlook) connectOutlook.style.display = available.has('microsoft') ? 'inline-flex' : 'none';
  if (authButtons) authButtons.style.display = anyAvailable ? 'flex' : 'none';
  if (authMsg) {
    authMsg.textContent = anyAvailable
      ? 'Please connect an account to begin.'
      : 'No email providers are configured for this deployment. Add Google or Microsoft credentials on the server to enable sign in.';
  }
  if (!serviceNameElem) {
    return;
  }
  if (payload.connected_service === 'none') {
    if (authView) authView.style.display = 'block';
    if (controls) controls.style.display = 'none';
...
# --- Microsoft Helpers ---
def _get_msal_app():
    if not (MS_CLIENT_ID and MS_CLIENT_SECRET and MS_AUTHORITY):
...
def _require_ms_token() -> str:
    ...
    if refresh_token:
        expires_on = token.get("expires_on") or token.get("expires_at")
        if expires_on:
            try:
                expires_on_ts = int(expires_on)
                if expires_on_ts - int(time.time()) < 60:
                    refresh_needed = True
            except ValueError:
                refresh_needed = False
...
- If the user asks to check, re-check, verify, refresh, look again, or otherwise confirm the state of their inbox or calendar, you MUST call the appropriate search/list tool before replying. Never rely only on previous summaries for those requests."""
            ...
            while self._active:
                try:
                    ...
                    if new_contacts:
                        ...
                        display_text, _ = _extract_suggestions(message)
    ...
    async def _after_bulk_mark_read(self, processed_ids: Set[str]) -> None:
        if not processed_ids:
            return
        self._handled_email_ids.clear()
        self._announced_unread_ids.clear()
        if self.current_email_context and self.current_email_context.get("id") in processed_ids:
    ...
    def tools(self):
        google_tools = [
            {"type": "function", "function": {"name": "gmail_search_emails", "description": "Searches for emails in the user's Gmail inbox.", "parameters": {"type": "object", "properties": {"query": {"type": "string"}, "max_results": {"type": "integer", "default": 5}}, "required": []}}},
            ...
            {"type": "function", "function": {"name": "gmail_mark_all_read", "description": "Marks every unread Gmail message in the inbox as read.", "parameters": {"type": "object", "properties": {}, "required": []}}},
        ...
        microsoft_tools = [
            ...
            {"type": "function", "function": {"name": "outlook_mark_all_read", "description": "Marks every unread Outlook message in the inbox as read.", "parameters": {"type": "object", "properties": {}, "required": []}}},
    ...
    async def gmail_mark_all_read(self) -> str:
        ...
        try:
            while loop_count < max_loops:
                ...
                if len(messages) < 500:
                    # likely no more unread messages; loop reiterates to confirm
                    continue
        except HttpError as exc:
            await self._after_bulk_mark_read(processed_ids)
            return f"Error marking Gmail messages as read: {exc}"

        if loop_count >= max_loops:
            count = len(processed_ids)
            await self._after_bulk_mark_read(processed_ids)
            if count:
                return f"Marked {count} Gmail messages as read, but reached the safety limit. Please run again for the rest."
            return "Reached the safety limit before finishing. Please try again."

        if processed_ids:
            await self._after_bulk_mark_read(processed_ids)
            return f"Marked {len(processed_ids)} unread Gmail emails as read."
        return "Your Gmail inbox is already clear."
    ...
    async def outlook_mark_all_read(self) -> str:
        processed_ids: Set[str] = set()
        failures = 0
        max_loops = 40
        loop_count = 0
        ...
        try:
            while loop_count < max_loops:
                ...
                response = await graph_request("GET", "/me/mailFolders('Inbox')/messages", params=params)
                ...
                for mid in ids:
                    try:
                        await graph_request("PATCH", f"/me/messages/{mid}", json={"isRead": True})
                        processed_ids.add(mid)
                    except RuntimeError as exc:
                        failures += 1
                        print(f"[OUTLOOK MARK ALL WARNING] Failed to update {mid}: {exc}")
                if len(ids) < batch_size:
                    continue
        except RuntimeError as exc:
            await self._after_bulk_mark_read(processed_ids)
            return f"Error marking Outlook messages as read: {exc}"

        if loop_count >= max_loops:
            count = len(processed_ids)
            await self._after_bulk_mark_read(processed_ids)
            extra_note = " Some messages might remain; please run the command again."
            if failures:
                extra_note += f" {failures} messages could not be updated."
            return f"Marked {count} Outlook emails as read.{extra_note}"

        if processed_ids:
            await self._after_bulk_mark_read(processed_ids)
            note = ""
            if failures:
                note = f" {failures} messages could not be updated."
            return f"Marked {len(processed_ids)} unread Outlook emails as read.{note}"
        if failures:
            return "No unread Outlook emails found, but a few updates failed."
        return "Your Outlook inbox is already clear."
    ...
    async def execute_tool_calls(self, tool_calls: List[Dict]):
        tool_functions = {
            ...
            "gmail_mark_all_read": self.gmail_mark_all_read,
            # Outlook
            ...
            "outlook_mark_all_read": self.outlook_mark_all_read,
            # Calendar (unified)
...
async def home(request: Request):
    session_id = _ensure_session_id(request.session)
    _get_session_state(session_id)
    return HTMLResponse(CONVERSATIONAL_HTML)

@app.get("/audio/{audio_id}")
async def get_audio(audio_id: str, request: Request, range: Optional[str] = Header(None)):
    session_id = request.session.get("session_id")
    if not session_id:
        return PlainTextResponse("Not Found", status_code=404)
    audio_data = _get_audio_bytes(session_id, audio_id)
    if not audio_data:
        return PlainTextResponse("Not Found", status_code=404)
...
async def websocket_endpoint(websocket: WebSocket):
    session_store = websocket.session if hasattr(websocket, "session") else websocket.scope.setdefault("session", {})
    ...
    with _session_scope(session_id):
        ...
        if isinstance(google_creds, Credentials) and getattr(google_creds, "valid", False):
            service = "google"
        else:
            ms_token = state.get("ms_token")
...
def get_auth_status(request: Request):
    ...
    if not session_id:
        session_id = _ensure_session_id(request.session)
    available = [name for name, enabled in AVAILABLE_SERVICES.items() if enabled]
    if session_id:
        state = _get_session_state(session_id)
        google_creds = state.get("google_creds")
        if isinstance(google_creds, Credentials) and getattr(google_creds, "valid", False):
            return {"connected_service": "google", "available_services": available}
        ms_token = state.get("ms_token") or {}
        if ms_token.get("access_token"):
            return {"connected_service": "microsoft", "available_services": available}
    return {"connected_service": "none", "available_services": available}

# Google OAuth
@app.get("/gmail/login")
def gmail_login(request: Request):
    if not AVAILABLE_SERVICES.get("google"):
...
async def gmail_oauth2callback(request: Request, code: str, state: str):
    ...
    session_id = _ensure_session_id(request.session)
    with _session_scope(session_id):
        _set_google_creds(flow.credentials)
        _clear_ms_token()
...
def outlook_login(request: Request):
    if not AVAILABLE_SERVICES.get("microsoft"):
        return PlainTextResponse("Microsoft OAuth is not configured.", status_code=503)
    _ensure_session_id(request.session)
