# app.py
"""
Conversational Gmail + Calendar Voice Assistant (Single File) - v5.6 (GPT-5-chat-latest Agentic)
...
- FIX v5.6: Injected current date and time into the system prompt for accurate relative time understanding (e.g., "tomorrow", "next week").
...
### CHANGED ### - Renamed to TEMPLATE and added a placeholder for the current time.
SYSTEM_PROMPT_TEMPLATE = """You are a helpful, conversational Google assistant for Gmail and Calendar.
- CRITICAL CONTEXT: The current date and time is {current_date_time}. Use this to resolve all relative time references like 'today', 'tomorrow', 'next Tuesday at 2pm', etc.
...
class ConversationManager:
    def __init__(self, ws: WebSocket):
        ...
        ### CHANGED ### - Dynamically create the system prompt with the current date and time.
        now = datetime.datetime.now()
        current_time_str = now.strftime("%A, %B %d, %Y, %I:%M %p %Z")
        dynamic_prompt = SYSTEM_PROMPT_TEMPLATE.format(current_date_time=current_time_str)
        
        self.history: List[Dict[str, Any]] = [{"role": "system", "content": dynamic_prompt}]
