...
function connectWebSocket(){
  return new Promise((resolve, reject) => {
    ...
    socket.onopen = () => { appendChat('system', 'Connection established. Assistant is starting...'); resolve(); };
...
class ConversationManager:
    def __init__(self, ws: WebSocket, service_type: str):
        ...
        base_instructions = """
- You are a world-class proactive voice assistant. Your goal is to help the user manage their digital life (emails, calendar) through natural conversation.
- Be proactive. On startup, you will be given a summary of new emails and today's events. Use this to form a helpful, engaging opening. For example: "Welcome back! You have 2 new emails and 1 meeting today. Where would you like to start?"
- Understand conversational context. If you say "You have an email from John Doe," and the user asks "What's it about?", you must understand they are asking about John's email.
- After calling a tool, ALWAYS summarize the results for the user in a conversational way. Don't just read out the raw data.
- When reading long emails, suggest summarizing.
- Keep responses concise and focused on the user's needs.
- Always ask clarifying questions if a request is ambiguous (e.g., "Which meeting do you want to move?")."""
    ...
    async def outlook_read_email(self, message_id: Optional[str] = None) -> str:
        ...
        r = await graph_request(
            "GET",
            f"/me/messages/{target_id}?$select=id,subject,from,bodyPreview,body",
            headers={"Prefer": 'outlook.body-content-type="text"'}
        )
        msg = r.json()
        from_addr = ((msg.get('from', {}) or {}).get('emailAddress', {}) or {}).get('address', '')
        self.current_email_context = {
            'id': msg.get('id', target_id),
            'from': from_addr,
    ...
    # --- AGENTIC CORE ---
    async def _get_unread_email_summary(self) -> str:
        try:
            ...
            if self.service_type == 'google':
                ...
                query = f"is:unread after:{after_date_str}"
                unread_json = await self.gmail_search_emails(query=query, max_results=5)
            else:
                params = {
                ...
                r = await graph_request("GET", "/me/messages", params=params)
                messages = r.json().get("value", [])
                if not messages: return "You have no new emails since yesterday."
                out = []
                for m in messages:
                    sender = (m.get('from', {}) or {}).get('emailAddress', {}) or {}
                    out.append({
            ...
            if "No emails found" in unread_json:
                return "You have no new emails since yesterday."

            emails = json.loads(unread_json)
            count = len(emails)
            plural = "s" if count > 1 else ""
            senders = list(set([e['from'].split('<')[0].strip() for e in emails[:3]]))
            senders_str = ", ".join(senders)
            return f"You have {count} new email{plural} since yesterday, including messages from {senders_str}."

        except Exception as e:
            print(f"[STARTUP ERROR] checking unread mail: {e}")
            return "Could not check for new emails."
    ...
    async def _get_startup_summary(self) -> str:
        try:
            email_summary, event_summary = await asyncio.gather(
